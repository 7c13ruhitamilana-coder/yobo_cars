...
DEFAULT_BIZ_ID = os.getenv("BIZ_ID") or SECRETS.get("BIZ_ID")
...
def validate_dates(start_date: str, end_date: str) -> tuple[bool, str | None]:
    try:
...
def api_fleet():
    ...
    if not biz_id:
        return jsonify({"ok": False, "error": "BIZ_ID not set"}), 400
    if not is_valid_uuid(biz_id):
        return jsonify({"ok": False, "error": "BIZ_ID must be a UUID"}), 400

    if not start_date or not end_date:
        return jsonify({"ok": False, "error": "start_date and end_date required"}), 400

    ok_dates, err = validate_dates(start_date, end_date)
    if not ok_dates:
...
def api_bookings():
    ...
    required = ["customer_name", "phone", "city", "start_date", "end_date", "car_id", "total_price", "location", "insurance"]
    ...
    try:
        ...
        return jsonify({"ok": False, "error": "Invalid car_id"}), 400
